...
// web-push se carga dinámicamente dentro de la función para evitar que su peso
// afecte el cold start de todas las demás server actions del dashboard.
export async function sendPushNotificationAction(
  ...
  if (!subs || subs.length === 0)
    return { error: 'No hay usuarios suscritos a esta tarjeta aún.' };

  const finalUrl = url || `https://vink.com/c/${card.slug}`;

  // Importación dinámica — web-push solo se instancia cuando se invoca esta acción
  ...
  for (const sub of subs) {
    ...
    try {
      const payload: Record<string, string> = { title, body, url: finalUrl };
      if (card.logo_url && !card.logo_url.startsWith('data:')) {
